<template>
  <div class="app-sidebar" :class="{'sidebar-show' : sidebarShow && isMobileWidth}">
    <h1>
      <button v-if="isMobileWidth" class="app-btn-mobile app-sidebar-btn-close" @click="hiddenSidebar">
    ...
    <form class="form" @submit.prevent="onSubmit">
      <div class="form-group">
        <label class="form-label required" for="name">Наименование товара</label>
        <input v-model="form.name" type="text" name="name" maxlength="40" placeholder="Введите наименование товара" class="form-control" :class="{'is-invalid': $v.form.name.$error}">
      ...
      <div class="form-group">
        <label class="form-label" for="description">Введите описание товара</label>
        <textarea v-model="form.description" name="description" placeholder="Введите описание товара" maxlength="200" class="form-control form-control--area"></textarea>
      </div>
      <div class="form-group">
        <label class="form-label required" for="url">Ссылка на изображение товара</label>
      ...
      <button class="app-btn app-btn-primary form-btn" type="submit" :disabled="invalid">Добавить товар</button>
    </form>
  </div>
</template>
